        ...
        <div className='w-[50rem] h-[20rem] bg-gray-100 mt-[7rem]'>
            <div className='w-full h-full grid grid-cols-[40%,60%] items-center'>
                ...
                <div className='flex justify-start items-center p-3'>
                    {movieDetail.showtimes.map((showtimes, id) => {
                        return <Link to={`/movie/reservation`} state={{showtime: showtimes.startTime, movie: movieDetail}}>
                            <button key={id} className="btn btn-success">{showtimes.startTime.toString()}</button>
                        </Link>
                    })}
                </div>
            </div>
