...
export const handlers = [
  ...
  rest.get<DefaultBodyType, {movieId: string}, Showtime[] | APIError>("/api/reservation/:movieId", (req, res, ctx) => {
    const movieId = parseInt(req.params.movieId, 10)
    const movie = movies.find(m => m.id ===  movieId)
    if (!movie) {
        return res(ctx.status(404), ctx.json({"message": "The movie does not exist."}))
    }
    return res(ctx.status(200), ctx.json(movie.showtimes))
  }),
  rest.get<DefaultBodyType, {movieId: string, showtimeId: string}, Seat[] | APIError>("/api/reservation/:movieId/:showtimeId", (req, res, ctx) => {
    const movieId = parseInt(req.params.movieId, 10)
    const movie = movies.find(m => m.id === movieId)
    if (!movie) {
        return res(ctx.status(404), ctx.json({"message": "The movie does not exist."}))
    ...
    if (!showtime) {
      return res(ctx.status(404), ctx.json({"message": "There is no showtime."}))
    }
